...
import type { LeaderboardCategory } from '../../types/api';
...
export function LeaderboardBento() {
  ...
  const currentCategory = categories.find((c) => c.id === selectedLeaderboardCategory);
  ...
  if (loading && !leaderboard) {
    return (
      <BentoCard colSpan={6} rowSpan={2} elevation={3} background="glass">
        <BentoCardHeader
          title="Leaderboard"
        ...
        <BentoCardContent>
          <div className="flex items-center justify-center py-8">
            <div className="text-foreground/60">Loading leaderboard...</div>
          </div>
        </BentoCardContent>
      </BentoCard>
    );
  ...
  return (
    <BentoCard colSpan={6} rowSpan={2} elevation={3} background="glass" className="overflow-hidden">
      <BentoCardHeader
        title="Leaderboard"
      ...
      <BentoCardContent>
        ...
        <div className="flex gap-2 mb-4">
          {categories.map((category) => (
            <Button
              key={category.id}
              variant={selectedLeaderboardCategory === category.id ? 'primary' : 'accent'}
              onPress={() => setLeaderboardCategory(category.id)}
              className="flex-1"
            >
        ...
        {!leaderboard || leaderboard.entries.length === 0 ? (
          <div className="text-center text-foreground/60 py-8">
            ...
            <p className="text-body-sm mt-2">Complete some jobs to see the leaderboard</p>
          </div>
        ) : (
          <>
